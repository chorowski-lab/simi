...
if __name__ == "__main__":
    ...
    pathClusteringCheckpoint = str(args.clustering_checkpoint)

    # '/pio/data/zerospeech2021/LibriSpeech/test-clean'
    pathDB = str(args.dataset)

    # '/pio/scratch/1/i290956/zs2021/clusterings/LibriSpeech/test-clean'
    pathOutputDir = str(args.output)

    seqNames, _ = findAllSeqs([pathDB], speakerLevel=args.speaker_level, extension=[args.file_ext], loadCache=False)
    ...
    # Load CluterModule
    if hasattr(clustering_args, 'seqNorm') and clustering_args.seqNorm:
        print("Using MVN")
    print("")
    ...
    if clustering_args.nullspace:
        print("Using null space")
    if hasattr(clustering_args, 'pathPCA') and clustering_args.pathPCA is not None:
        print("Using PCA")
    featureMaker = loadCPCFeatureMaker(
                        ...
                        keep_hidden=False,
                        load_nullspace=clustering_args.nullspace, 
                        pcaPath=clustering_args.pathPCA if hasattr(clustering_args, 'pathPCA') else None)
    ...
    def cpc_feature_function(x):
        return buildFeature_batch(featureMaker, x, seqNorm=hasattr(clustering_args, 'seqNorm') and clustering_args.seqNorm, 
                                  strict=True, maxSizeSeq=10240, batch_size=8)
